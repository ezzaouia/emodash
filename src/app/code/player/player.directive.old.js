    ...
    .directive('player', function () {
        ...
        var playerSliderActions = function (player) {
            ...
            function _stopPlayChunk() {
                ...
                _playChunkData = null;
                player.emit('playChunk:stop', _.pick(oldData, 'startTime', 'stopTime', 'datasetKey', 'itemIndex'));
            }
            ...
            function seek(h) {
                ...
                d3.select('.track-buffer-progress').attr('x2', player.xScale(player._calcBufferChunkEnd()));
            ...
            return {
                ...
                changeVolume: function volumeUp() {
                    ...
                    player.$log.debug('changeVolume fired');
                ...
                playChunk: function playChunk(startTime, stopTime, eventData) {
                    ...
                    player.$log.debug('playChunk fired', startTime, stopTime);
                    ...
                    _playChunkData.on(player.stream, 'seeking', function () {
                        if ((player.stream.currentTime > stopTime) || (player.stream.currentTime < startTime)) {
                            player.$log.debug('seeking cancel playChunk');
                            _stopPlayChunk();
                        }
                    });
                    ...
                    _playChunkData.on(player.stream, 'timeupdate', function () {
                        if (player.stream.currentTime > stopTime) {
                            player.$log.debug('playChunk stop');
                            _stopPlayChunk();
                ...
                mute: function mute() {
                    ...
                    d3.select('.vc-slider-track-progress').attr('x2', player.xVolumeScale(player.volume));
        ...
        var playerSliderEvents = function (player) {
            ...
            return {
                ...
                mouseleave: function () {
                    ...
                    d3.selectAll('.track-cursor,.track,.track-progress,.track-buffer-progress')
                ...
                mousemove: function () {
                    ...
                    d3.select('#player-tooltip')
                        ...
                        .style('left', (d3.mouse(d3.select('#player').node())[0] - jQuery('#player-tooltip').width() / 2) + 'px');
                },
                dblclick: function () {
                    var event = {
        ...
        var volumeSliderEvents = function (player) {
            ...
            return {
                ...
                mouseleave: function () {
                    d3.select(this).style('cursor', 'default');
                    d3.selectAll('.vc-slider-track-cursor,.vc-slider-track,.vc-slider-track-progress')
                        .style('stroke-width', 1);
                    d3.select('.vc-slider-track-cursor')
                ...
                mousemove: function () {
                    ...
                    d3.select('.vc-slider-track-cursor')
                        ...
                        .attr('x2', player.xVolumeScale(player.xVolumeScale.invert(d3.mouse(this)[0])));
                },
            };
        };

        function PlayerController(ControllerUtils, $log, $scope, $timeout, $window, $document, $translate) {
            'ngInject';
            ...
            this.$log = $log;
            this.timeDurationFilter = function (streamTime) {
            ...
            this.$root = angular.element(document.getElementById('player-root'));
        ...
        PlayerController.prototype._initialLayout = function () {
            ...
            this.UiCfg.MARGIN = _.extend({ top: 0, bottom: 0, left: 8, right: 8, datasetLabelLeft: 0, volumeSlider: 20 }, _.get(player, 'src.options.margin', {}));

            var playerTimeLineCssClasses = '.track,.track-buffer-progress,.track-cursor,.track-overlay,.track-progress,.vc-slider-track,.vc-slider-track-cursor,.vc-slider-track-overlay,.vc-slider-track-progress';

            var DATASETS_PLAYER_PAD = _.get(player, 'src.options.playerSliderDatasetsPad', 0);
            var PLAYER_SLIDER_HEIGHT = _.get(player, 'src.options.playerSlider.playerSliderHeight', 30);
            var PLAYER_CONTROLS_HEIGHT = _.get(player, 'src.options.playerSlider.playerVolumeHeight', 40);
            var VOLUME_SLIDER_WIDTH = _.get(player, 'src.options.playerSlider.playerVolumeWidth', 140);
            ...
            _.each(_.get(player, 'src.options.encoding'), function (item, itDatasetKey) {
                ...
                if (dtHeight) {
                    ...
                    player.mapDatasetsHeight[itDatasetKey] = _.get(player, 'src.options.datasetHeight');
                }
            });

            var datasetsHeight = _.sum(_.values(player.mapDatasetsHeight)) + DATASETS_PLAYER_PAD;
            var width = _.get(player, 'src.options.width', jQuery('#player-root').width());
            var height = PLAYER_SLIDER_HEIGHT + datasetsHeight + player.UiCfg.MARGIN.top + player.UiCfg.MARGIN.bottom;
            ...
            var gSvg = d3.select('#player')
                ...
                .attr('transform', 'translate(' + player.UiCfg.MARGIN.left + ',' + player.UiCfg.MARGIN.top + ')');

            // group datasets
            player.gDatasets = gSvg.append('g')
                .attr('transform', 'translate(0,0)')
                .classed('g-player-datasets', true);

            // group player-slider
            player.gPlayerSlider = gSvg.append('g')
                .attr('transform', 'translate(0,' + (datasetsHeight + PLAYER_SLIDER_HEIGHT / 2) + ')')
            ...
            var playerSlider = player.gPlayerSlider.append('line')
                .attr('class', 'track')
                ...
                .call(d3.drag()
                    .on('start.interrupt', function () {
                        player.$log.debug('[playerStreamReport] start.interrupt');
            ...
            this.gPlayerSlider.insert('circle', '.track-overlay')
            ...
            d3.selectAll('.track-progress,.track-buffer-progress,.track-cursor').attr('x2', player.xScale.range()[0]);
            ...
            // x volume scale
            var xVolumeScale = d3.scaleLinear()
                .domain([0, 1])
            ...
            // volume svg
            var volumeControlGroup = d3.select('#volume-control-svg')
                .append('svg')
                .attr('width', 0 /*VOLUME_SLIDER_WIDTH*/)
                .attr('height', PLAYER_CONTROLS_HEIGHT)
                .append('g')
                .classed('g-player', true)
                .attr('transform', 'translate(' + player.UiCfg.MARGIN.volumeSlider + ', ' + (PLAYER_CONTROLS_HEIGHT / 2) + ')');

            var volumeControlSlider = volumeControlGroup.append('line')
            ...
            volumeControlGroup.insert('circle', '.vc-slider-track-overlay')
        ...
        PlayerController.prototype._checkReadyState = function () {
            ...
            if (fixedWidth) {
                player.$log.debug('Ready via fixed with');
                this._initialLayout();
                return;
            }

            if (jQuery('#player-root').width() <= 0) {
        ...
        PlayerController.prototype._onStreamDataAvailable = function () {
            ...
            function _updateBuffering() {
                ...
                d3.select('.track-buffer-progress').attr('x1', player.xScale(player.stream.currentTime));
                ...
                if (player.isDebug) {
                    ...
                    player.$log.debug('player.src.datasets.debugAudioBuffer', player.src.datasets.debugAudioBuffer);
                }
            }
        ...
        PlayerController.prototype._onResize = function () {
            var player = this;
            // recompute the with
            var width = jQuery('#player-root').width();
            // refresh our scale
            player.xScale.range([0, width - player.UiCfg.MARGIN.left - player.UiCfg.MARGIN.right]);
            // update the with of our svg
            d3.select('#player').select('#player-svg').select('svg').attr('width', width);
            // update the x1,x2 of our lines (x1,y1|------------|x2,y2)
            this.gPlayerSlider.selectAll('.track, .track-overlay').attr('x2', player.xScale.range()[1]);

            this.gPlayerSlider.select('.track-progress').attr('x2', player.xScale(player.stream.currentTime));

            this.gPlayerSlider.select('.track-cursor').attr('x2', player.xScale.range()[0]);

            this.gPlayerSlider.select('.track-buffer-progress').attr('x1', player.xScale(player.stream.currentTime));
            this.gPlayerSlider.select('.track-buffer-progress').attr('x2', player.xScale(player._calcBufferChunkEnd()));
        ...
        PlayerController.prototype._renderDatasets = function () {
            ...
            _.each(player.src.datasets, function (itDatasetValue, itDatasetKey) {
                ...
                var gPlayerDataset = _.get(datasetOption, 'svgGroupClass') ? _.get(datasetOption, 'svgGroupClass') : 'g-player-' + itDatasetKey;
                ...
                if (_.get(datasetOption, 'display.child')) {
                    ...
                    player.gDatasets
                        .append('g')
                        .attr('transform', function (/*d, i*/) { return 'translate(0, ' + (dtsHeightCumulator + dtHeight / 2) + ')'; })
                ...
                if (_.get(datasetOption, 'inline.show')) {
                    addDatasetInline(d3.select(renderingGroupClass), player.xScale.range()[0], player.xScale.range()[1], _.get(datasetOption, 'inline.cssclass'));
                }
                ...
                if (_.get(datasetOption, 'label.show')) {
                    ...
                    addDatasetLabel(d3.select(renderingGroupClass), labelWithSize, x, y, _.get(datasetOption, 'label.cssclass'));
                }
                ...
                if (_.get(datasetOption, 'events.click', true) === false) {
                    genericClickHandler = function () { return null; };
                } else {
                    genericClickHandler = player._onElementClickEvent.bind(player, datasetOption, itDatasetKey);
                }
                if (_.get(datasetOption, 'events.mouseenter', false) === false) {
                    genericMouseenterHandler = function () { return null; };
                } else {
                    genericMouseenterHandler = player._onElementMouseEvent.bind(player, 'mouseenter', datasetOption, itDatasetKey);
                }
                if (_.get(datasetOption, 'events.mouseleave', false) === false) {
                    genericMouseleaveHandler = function () { return null; };
                } else {
                    genericMouseleaveHandler = player._onElementMouseEvent.bind(player, 'mouseleave', datasetOption, itDatasetKey);
                }

                _.each(symbols, function (symbol, iSymbol) {
                    // d3 data binding
                    var d3Key = itDatasetKey + '.' + iSymbol;
                    renderedDatasetsSelection[d3Key] = d3.select(renderingGroupClass)
                    ...
                    switch (_.get(symbol, 'type')) {
                        case 'circle':
                            renderedDatasetsSelection[d3Key].enter()
                                .append('circle')
                                .merge(renderedDatasetsSelection[d3Key])
                                .attr('cx', getXPosition)
                                .attr('cy', getYPosition.bind(null, iSymbol))
                                .attr('r', getSymbol.bind(null, iSymbol))
                                .attr('class', function (d) { return getCssClass(iSymbol, d) + ' dtelement-' + itDatasetKey + '-' + iSymbol; })
                        ...
                        case 'line':
                            renderedDatasetsSelection[d3Key].enter()
                                ...
                                .attr('class', function (d) {
                                    return getCssClass(iSymbol, d) + ' dtelement-' + itDatasetKey + '-' + iSymbol;
                                })
                        ...
                        case 'stackedArea':
                            ...
                            var yscale = d3.scaleLinear().domain(domain).range(range);
                            var streamArea = d3.area()
                                .x(function (d) { return player.xScale(_.get(d, value)); })
                            ...
                            renderedDatasetsSelection[d3Key]
                                ...
                                .enter()
                                .append('g')
                                .attr('class', function (d) { return 'emotion cursor-pointer ' + _.get(d, 'key'); })
                                .append('path')
                                .attr('class', cssclass)
                                .attr('transform', getTransform.bind(null, iSymbol))
                                .attr('d', streamArea)
                                ...
                                .on('click', function (/*d, i*/) {
                                    ...
                                    player._handlePlayerSliderDrag(player.xScale.invert(d3.mouse(this)[0]));
                                })
                                .on('mouseenter', function (d/*, i*/) {
                                    var currentColor = _.get(colorsToTexture, _.get(d, 'key'));
                                    var texture = _.get(player, 'colorsTexturesMap' + '.' + d3.color(currentColor) + '.' + 't');
            ...
            function getTransform(iSymbol, d) {
                ...
                if (!theta) { return translateTransform; }
            ...
            function generateTextures(colors) {
                ...
                var colorsVals = _.values(colors), texture;
                colorsVals = _.uniq(colorsVals);
                _.each(colorsVals, function (color) {
                    texture = textures.lines().size(4).strokeWidth(2).stroke(color).background('#fff');
                    _.set(player, 'colorsTexturesMap' + '.' + d3.color(color), { color: color, t: texture });
        ...
        PlayerController.prototype._onElementClickEvent = function (option, datasetKey, d, i) {
            ...
            if (!value.hasOwnProperty('startkey') || !value.hasOwnProperty('stopkey')) {
                player.$log.debug('No start/stop');
                return;
            }
            player.playerActions.playChunk(_.get(d, value.startkey), _.get(d, value.stopkey), event);
        };
        ...
        PlayerController.prototype._onElementMouseEvent = function (eventName, option, datasetKey, d, i) {
            player.$log.debug('[tooltip] elementMouseEvent emit');
        ...
        return {
            ...
            link: function (scope, element, attributes, controller) {
                ...
                function initClientStream() {
                    controller.$log.debug('[initClientStream] start');
                    controller.stream = document.getElementById('client-stream');
                    if (!controller.stream) {
                        controller.$log.debug('[initClientStream] !controller.stream');
                        controller.$timeout(initClientStream, 0);
                        return;
                    }
                    controller.$log.debug('[initClientStream] controller.stream');
                    controller.stream.onloadeddata = controller._onStreamDataAvailable.bind(controller);
                }
            },
        };
    });
